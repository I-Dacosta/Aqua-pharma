          ...
          <ScrollReveal duration={0.7} yOffset={14} staggerChildren staggerAmount={0.05} start="top 94%">
            <div className="divide-y divide-(--brand-blue)/10 border-y border-(--brand-blue)/10">
              {product.stats.map((stat) => (
                <div
                  key={stat.label}
                  className="grid grid-cols-[minmax(9.5rem,max-content)_minmax(0,1fr)] items-center gap-6 bg-white/48 py-5 md:grid-cols-[minmax(12rem,max-content)_minmax(0,1fr)] md:py-6"
                >
                  <p className="whitespace-nowrap font-heading text-[clamp(2rem,2.4vw,2.8rem)] font-light leading-none text-(--brand-blue)">
                    {stat.value}
                  </p>
